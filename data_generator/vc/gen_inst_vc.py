...
def generate_vc(graph, filename):
    """
    ...
    with open(filename, 'w') as lp_file:
        ...
        lp_file.write("\nsubject to\n")
        for count, (n1, n2) in enumerate(sorted(list(graph.edges))):
            lp_file.write(f"C{count+1}:" + f"x{n1+1} + x{n2+1} >= 1\n")
...
def solve_vc(lp_path, time_limit=200):
...
def gen_vc(data_dir, ninst, scale_lower, scale_upper=None, solve=True):
    ...
    for i in range(ninst):
        ...
        generate_vc(graph, lp_path)
        
        if solve:
            sol_path = solve_vc(lp_path)

if __name__ == '__main__':

    home = expanduser("~")
    # data_dir = os.path.join(home, f'storage1/instances/vc/train_500-1000')
    # gen_vc(data_dir, 1000, 500, 1000, solve=True)

    data_dir = os.path.join(home, f'storage1/instances/vc/time_1000')
    gen_vc(data_dir, 30, 1000, solve=False)

    data_dir = os.path.join(home, f'storage1/instances/vc/time_3000')
    gen_vc(data_dir, 30, 3000, solve=False)

    data_dir = os.path.join(home, f'storage1/instances/vc/time_5000')
    gen_vc(data_dir, 30, 5000, solve=False)

    data_dir = os.path.join(home, f'storage1/instances/vc/time_7000')
    gen_vc(data_dir, 30, 7000, solve=False)

    data_dir = os.path.join(home, f'storage1/instances/vc/time_9000')
    gen_vc(data_dir, 30, 9000, solve=False)